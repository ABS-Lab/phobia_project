...
df3_extra = @pipe dfpre |> 
	groupby(_,:participant_num) |> 
	combine(_, [:spider_phobia, :video_scr, :video_hp, :resp_arousal, :resp_fear, :resp_valence] .=> mean_miss) 
...
# ╠═╡ disabled = true
#=╠═╡
CSV.write("phobiacordfspider.csv", cordfextra)
  ╠═╡ =#
...
mm15 = fit(LinearMixedModel, @formula(resp_fear ~ spider_phobia * video_scr_cmc + spider_phobia *  video_hp_cmc + spider_phobia * resp_arousal_cmc + spider_phobia * resp_valence_cmc + video_scr_mean + video_hp_mean + resp_arousal_mean + resp_valence_mean + (video_scr_cmc + video_hp_cmc + resp_arousal_cmc + resp_valence_cmc|participant_num_str)), dfcleancol2)
...
# ╔═╡ 625ee0de-38b0-48c3-ba1f-034091e0fb5a
