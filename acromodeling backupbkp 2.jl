...
using CSV, DataFrames, Plots, Random, Statistics, GLM, StatsPlots, Tables, Printf, MixedModels, Pipe
...
df1pre = dfpre[dfpre[:, :video_condition] .== 3, :]

# ╔═╡ 313f2cd7-9dd6-462c-83e6-86232e8dfa9e
df2pre_pre = @pipe dfpre |> 
	groupby(_,:participant_num) |> 
	transform(_, [:video_scr, :resp_arousal, :resp_fear, :resp_valence] .=> mean, [:video_scr, :resp_arousal, :resp_fear, :resp_valence] .=> std) |> 
	transform(_, [:video_scr, :video_scr_mean]=> ByRow(-) => :video_scr_cmc, [:resp_arousal, :resp_arousal_mean]=> ByRow(-) => :resp_arousal_cmc, [:resp_fear, :resp_fear_mean]=> ByRow(-) => :resp_fear_cmc, [:resp_valence, :resp_valence_mean]=> ByRow(-) => :resp_valence_cmc) |>
	transform(_, [:video_scr_cmc, :video_scr_std]=> ByRow(/) => :video_scr_z, [:resp_arousal_cmc, :resp_arousal_std]=> ByRow(/) => :resp_arousal_z, [:resp_fear_cmc, :resp_fear_std]=> ByRow(/) => :resp_fear_z,
	[:resp_valence_cmc, :resp_valence_std]=> ByRow(/) => :resp_valence_z)

# ╔═╡ 2d0f1009-7f92-4f91-bc26-2a7cd5b4ec5e
df2pre = df2pre_pre[df2pre_pre[:, :video_condition] .== 3, :]
...
# ╔═╡ 7b7ca73f-9fd2-48ce-89d1-ef3e55d18962
# ╠═╡ disabled = true
#=╠═╡
dfcleancol[:, :participant_num_str] = dfcleancol[:, :participant_num] .|> string
  ╠═╡ =#

# ╔═╡ a18c6b92-9c18-4edd-b4ef-5e7de0a1784a
# ╠═╡ disabled = true
#=╠═╡
df2cleancol[:, :participant_num_str] = df2cleancol[:, :participant_num] .|> string
  ╠═╡ =#

# ╔═╡ 409c55aa-a290-4403-a707-42e6b39c1943
dfcleancol[dfcleancol[:,:participant_num] .== 224,:]

# ╔═╡ a4b5f290-32fa-41c4-a44f-c09ed7b106cf
# ╠═╡ disabled = true
#=╠═╡
gdf = groupby(dfcleancol, [:participant_num, :video])
  ╠═╡ =#
...
function comparevar(var1) #with phobia correlation
	return cor(dfcleancol[:,"spider_phobia"],dfcleancol[:,var1])
end
...
df2_subjlvl = combine(gdf, [:video_scr, :resp_arousal] => meantest, :spider_phobia => mean, renamecols=false) |> dropmissing

# ╔═╡ 4d81be85-1611-41d6-907b-dcf411143162
scatter(df2_subjlvl.video_scr_resp_arousal .|> x -> x[1], df2_subjlvl.spider_phobia, label="Intercept")

# ╔═╡ 412538a0-2bb3-4e5d-a78c-65dfdcbbaa08
scatter(df2_subjlvl.video_scr_resp_arousal .|> x -> x[2], df2_subjlvl.spider_phobia, label="Weight")

# ╔═╡ 19195589-73d3-48ae-9553-0c5ebaf0effe
scr_to_aro_int = cor(df2_subjlvl.video_scr_resp_arousal .|> x -> x[1], df2_subjlvl.spider_phobia)

# ╔═╡ 2df570fc-0f9e-4166-82d8-83fcdb115d2f
scr_to_aro_coef = cor(df2_subjlvl.video_scr_resp_arousal .|> x -> x[2], df2_subjlvl.spider_phobia)
...
df2_subjlvl_2 = combine(gdf, [:resp_arousal, :resp_fear] => meantest, :spider_phobia => mean, renamecols=false) |> dropmissing

# ╔═╡ d0aefb0a-7ae2-43f7-a0ea-fdf5662c2ca1
scatter(df2_subjlvl_2.resp_arousal_resp_fear .|> x -> x[1], df2_subjlvl.spider_phobia, label="Intercept")

# ╔═╡ 39f10fb6-0383-4742-8ebf-fd0969aeda3c
scatter(df2_subjlvl_2.resp_arousal_resp_fear .|> x -> x[2], df2_subjlvl_2.spider_phobia, label="Weight")

# ╔═╡ 0c5fef51-9132-4a37-bfc3-ff042a82acc5
aro_to_fear_int = cor(df2_subjlvl_2.resp_arousal_resp_fear .|> x -> x[1], df2_subjlvl_2.spider_phobia)

# ╔═╡ 1fa45402-b95c-4cbb-9eb3-dc1e3ec169a8
aro_to_fear_coef = cor(df2_subjlvl_2.resp_arousal_resp_fear .|> x -> x[2], df2_subjlvl_2.spider_phobia)
...
df2_subjlvl_hp = combine(gdf, :spider_phobia => mean, renamecols=false);
...
# ╔═╡ 51000871-21f4-4b95-96b2-de96f4aa3698
# ╠═╡ disabled = true
#=╠═╡
dfcleancol2_pre = @pipe dfcleancol |> 
            groupby(_,:participant_num_str) |> # group by country
            transform(_, :video_scr => mean => :video_scr_mean)|> # add a new variable income_mean :video_scr, :resp_arousal
            transform(_, [:video_scr, :video_scr_mean]=> ByRow(-) => :video_scr_cmc);  # add a new variable the centered variable

  ╠═╡ =#

# ╔═╡ 9723dcd6-fc61-4867-b2ed-5ab411bcbfb2
# ╠═╡ disabled = true
#=╠═╡
dfcleancol2_pre2 = @pipe dfcleancol2_pre |> 
            groupby(_,:participant_num_str) |> # group by country
			transform(_, :resp_arousal => mean => :resp_arousal_mean)|>
			transform(_, [:resp_arousal, :resp_arousal_mean]=> ByRow(-) => :resp_arousal_cmc);

  ╠═╡ =#

# ╔═╡ 13592f7c-faea-4278-9719-e5fdc9046be8
# ╠═╡ disabled = true
#=╠═╡
dfcleancol2_pre3 = @pipe dfcleancol2_pre2 |> 
            groupby(_,:participant_num_str) |> # group by country
            transform(_, :scr_change_video => mean => :scr_change_video_mean)|> # add a new variable income_mean :video_scr, :resp_arousal
            transform(_, [:scr_change_video, :scr_change_video_mean]=> ByRow(-) => :scr_change_video_cmc);  # add a new variable the centered variable


  ╠═╡ =#

# ╔═╡ 7d0d38ae-ba31-4301-885d-b682e1728592
# ╠═╡ disabled = true
#=╠═╡
dfcleancol2_OLD = @pipe dfcleancol2_pre3 |> 
            groupby(_,:participant_num_str) |> # group by country
			transform(_, :resp_valence => mean => :resp_valence_mean)|>
			transform(_, [:resp_valence, :resp_valence_mean]=> ByRow(-) => :resp_valence_cmc)
...
dfcleancol2 = @pipe dfcleancol |> 
            ...
            transform(_, [:video_scr, :resp_arousal, :scr_change_video, :resp_valence] .=> mean)|> 
            transform(_, [:video_scr, :video_scr_mean] => ByRow(-) => :video_scr_cmc, 				[:resp_arousal, :resp_arousal_mean] => ByRow(-) => :resp_arousal_cmc, 
...
# ╔═╡ a5c2e3f7-901d-48e7-ba9a-b72bc23a2d52
# ╠═╡ disabled = true
#=╠═╡
ranef(mm3)[1][2,:];
  ╠═╡ =#

# ╔═╡ 63e47ef1-2aec-4e42-9b93-189f0af5ed09
# ╠═╡ disabled = true
#=╠═╡
DataFrame(only(raneftables(mm3)));
  ╠═╡ =#

# ╔═╡ 1da4269e-ee2d-49b8-b537-248f912940cf
mm3coef_i = cor(ranef(mm3)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 0822037d-4284-4e00-8a35-9c2de8f6f4e3
mm3coef_w = cor(ranef(mm3)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 41cb0ff6-35ee-49e8-9b31-962e733bda44
scatter(ranef(mm3)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Src to arousal (cor=$(get_trunc(mm3coef_i)))", xlabel="SCR cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ 4100e32b-4b8d-4e02-8f3f-d994169a8c51
scatter( df2_subjlvl_hp.spider_phobia,ranef(mm3)[1][1,:], label="participant", title="Src to arousal (cor=$(get_trunc(mm3coef_i)))", ylabel="SCR cluster mean centering MLM raneff Intercept", xlabel="Heights Phobia")

# ╔═╡ c7d2bc63-47b4-45c6-aa5c-c1c890c4f502
scatter(ranef(mm3)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Src to arousal (cor=$(get_trunc(mm3coef_w)))", xlabel="SCR cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
# ╔═╡ 82c1d7ec-7cb1-4eaf-8366-ea80de92c3f6
# ╠═╡ disabled = true
#=╠═╡
DataFrame(only(raneftables(mm4)));
  ╠═╡ =#

# ╔═╡ b89451ce-8cd9-46e4-9f4d-a883fd382b4f
mm4coef_i = cor(ranef(mm4)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 97497c87-9145-4655-980e-0cb9b6157397
mm4coef_w = cor(ranef(mm4)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 9218c7d0-5be6-4e8b-bdc2-5b4b1514c7d5
scatter(ranef(mm4)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Arousal to Fear (cor=$(get_trunc(mm4coef_i)))", xlabel="Arousal cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ 7946b928-1d29-43e9-8315-871b63bd49c5
scatter(ranef(mm4)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Arousal to Fear (cor=$(get_trunc(mm4coef_w)))", xlabel="Arousal cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
mm6coef_i = cor(ranef(mm6)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 709a660d-df99-4c0e-b43c-b2ddd5ae3f87
mm6coef_w = cor(ranef(mm6)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ da14faf8-5d11-4b7d-a9f1-d82f4c1a73e2
scatter(ranef(mm6)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr to fear (cor=$(get_trunc(mm6coef_i)))", xlabel="SCR cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ a4e8582c-9f88-4c61-8f86-120930344057
scatter(ranef(mm6)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr to fear (cor=$(get_trunc(mm6coef_w)))", xlabel="SCR cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
mm12coef_i = cor(ranef(mm12)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 4d61d49d-3247-4501-8022-c946124538da
mm12coef_w = cor(ranef(mm12)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 379eb0cf-1bed-4412-8a82-272ccf4ec172
scatter(ranef(mm12)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Src to valence (cor=$(get_trunc(mm12coef_i)))", xlabel="SCR cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ a4ebcb4a-8f79-44b7-9984-c7213f606f0f
scatter(ranef(mm12)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Src to valence (cor=$(get_trunc(mm12coef_w)))", xlabel="SCR cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
# ╔═╡ 588d81f3-08de-401a-9180-f6e64149b565
# ╠═╡ disabled = true
#=╠═╡
DataFrame(only(raneftables(mm5)));
  ╠═╡ =#
...
mm5coef_i = cor(ranef(mm5)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 50adebf3-9a30-4e0b-af51-0cd645c0e351
mm5coef_w = cor(ranef(mm5)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ 2704cbb8-55ca-45fc-9a43-f713c6a19650
scatter(ranef(mm5)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr change to arousal (cor=$(get_trunc(mm5coef_i)))", xlabel="SCR change cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ 47f40223-b93f-406a-9b38-494ff469c72c
scatter(ranef(mm5)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr change to arousal (cor=$(get_trunc(mm5coef_w)))", xlabel="SCR change cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
mm7coef_i = cor(ranef(mm7)[1][1,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ db89dd86-fa57-4f0a-acb2-5fc1a872c28f
mm7coef_w = cor(ranef(mm7)[1][2,:], df2_subjlvl_hp.spider_phobia);

# ╔═╡ baee8637-8a47-4438-bbcc-05c88f6117fa
scatter(ranef(mm7)[1][1,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr change to fear (cor=$(get_trunc(mm7coef_i)))", xlabel="SCR change cluster mean centering MLM raneff Intercept", ylabel="Heights Phobia")

# ╔═╡ fe1dff56-5ab2-4b1b-9014-d4524ac536f4
scatter(ranef(mm7)[1][2,:], df2_subjlvl_hp.spider_phobia, label="participant", title="Scr change to fear (cor=$(get_trunc(mm7coef_w)))", xlabel="SCR change cluster mean centering MLM raneff Weight", ylabel="Heights Phobia")
...
mm8 = fit(LinearMixedModel, @formula(resp_arousal ~ video_scr_cmc * spider_phobia + video_scr_mean + (video_scr_cmc|participant_num_str)), dfcleancol2)
...
mm8coef_i = cor(ranef(mm8)[1][1,:], df2_subjlvl_hp.spider_phobia)

# ╔═╡ 509167a3-5aa8-4871-a838-16fd75225a6e
mm8coef_w = cor(ranef(mm8)[1][2,:], df2_subjlvl_hp.spider_phobia)

# ╔═╡ 008ea8e8-0500-4a58-8374-ff6670cd2cd4
mm9= fit(LinearMixedModel, @formula(resp_fear ~ resp_arousal_cmc * spider_phobia + resp_arousal_mean + (resp_arousal_cmc|participant_num_str)), dfcleancol2)

# ╔═╡ 92523847-a322-425f-9e41-f6799f4bfae9
loglikelihood(mm9)

# ╔═╡ 6c511ad7-a325-41fa-a63f-86292cbd58d0
mm10= fit(LinearMixedModel, @formula(resp_fear ~ video_scr_cmc * spider_phobia + video_scr_mean + (video_scr_cmc|participant_num_str)), dfcleancol2)

# ╔═╡ fda8f196-e9b0-4e1b-a4d0-b61109a8769c
loglikelihood(mm10)

# ╔═╡ 5f5bccbb-fd01-4806-8dd0-b729f34e04d7
mm11= fit(LinearMixedModel, @formula(resp_fear ~ video_scr_cmc * spider_phobia * resp_arousal_cmc + video_scr_mean + resp_arousal_mean +  (video_scr_cmc * resp_arousal_cmc|participant_num_str)), dfcleancol2)

# ╔═╡ 46ab0a1a-0716-4400-be3d-85f401f04310
mm13= fit(LinearMixedModel, @formula(resp_fear ~  spider_phobia * video_scr_cmc * resp_arousal_cmc * resp_valence_cmc + video_scr_mean + resp_arousal_mean +  resp_valence_mean + (video_scr_cmc * resp_arousal_cmc * resp_valence_cmc|participant_num_str)), dfcleancol2)

# ╔═╡ 88d2c0ec-6b3d-4388-98f8-248007a3555e
loglikelihood(mm11)

# ╔═╡ 4ea85f71-91d6-4c41-a323-5790217c27c0
loglikelihood(mm13)

# ╔═╡ bf43d604-e9ec-41f3-9855-d9c56bb0e57b
aic(mm11)

# ╔═╡ 79b1b83d-d8ec-4cf9-974d-58f1852e7ae3
aic(mm13)
...
df2_subjlvl_hp.spider_phobia

# ╔═╡ 2ec4d4c9-a20a-4194-8f3c-d66b0989fd12
std(df2_subjlvl_hp.spider_phobia)

# ╔═╡ f64bd09e-9652-40a4-894c-98fdb9559c63
std_up = mean(df2_subjlvl_hp.spider_phobia) + std(df2_subjlvl_hp.spider_phobia)

# ╔═╡ 3284ffe2-82c2-4422-a05c-e8993bf2dc1d
bm_std_up = df2_subjlvl_hp.spider_phobia .> std_up

# ╔═╡ ec3fb584-b839-4402-95e0-cb2f23140418
mean(randeff[1,bm_std_up])

# ╔═╡ 21c5b893-3f56-43ec-bd3c-24708c915656
std_dwn = mean(df2_subjlvl_hp.spider_phobia) - std(df2_subjlvl_hp.spider_phobia)

# ╔═╡ 7d1a511f-c0c1-4412-b644-04961560cc68
bm_std_dwn = df2_subjlvl_hp.spider_phobia .< std_dwn
...
hparr = combine(gdf3, :spider_phobia => mean, renamecols=false).spider_phobia;
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "2b9bed6edaedb8ed72fa36f14eb85a0852756eda"
...
git-tree-sha1 = "8e297ed1e89e8c2f3518b829ba4ad05e2eae31af"
