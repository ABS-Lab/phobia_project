...
df2pre = df2pre_pre[df2pre_pre[:, :video_condition] .== 1, :]
...
mm13= fit(LinearMixedModel, @formula(resp_fear ~ video_scr_cmc * heights_phobia * resp_arousal_cmc * resp_valence_cmc + video_scr_mean + resp_arousal_mean +  (video_scr_cmc * resp_arousal_cmc * resp_valence_cmc|participant_num_str)), dfcleancol2)
