...
df3_extra = @pipe dfpre |> 
	groupby(_,:participant_num) |> 
	combine(_, [:social_phobia, :video_scr, :video_hp, :resp_arousal, :resp_fear, :resp_valence] .=> mean_miss) 
...
# ╠═╡ disabled = true
#=╠═╡
CSV.write("phobiacordfsocial.csv", cordfextra)
  ╠═╡ =#
...
mm15 = fit(LinearMixedModel, @formula(resp_fear ~ social_phobia * video_scr_cmc + social_phobia *  video_hp_cmc + social_phobia * resp_arousal_cmc + social_phobia * resp_valence_cmc + video_scr_mean + video_hp_mean + resp_arousal_mean + resp_valence_mean + (video_scr_cmc + video_hp_cmc + resp_arousal_cmc + resp_valence_cmc|participant_num_str)), dfcleancol2)
